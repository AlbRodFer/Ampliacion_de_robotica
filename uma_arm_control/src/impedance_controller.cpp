    ...
    */
    ...
    private:
        ...
        Eigen::VectorXd forward_kinematics()
        {
            ...
            Eigen::VectorXd x(2);
            x << l1_ * cos(joint_positions_(0)) + l2_ * cos(joint_positions_(0) + joint_positions_(1)),
                 l1_ * sin(joint_positions_(0)) + l2_ * sin(joint_positions_(0) + joint_positions_(1));
        ...
        void update_jacobians()
        {
            ...
            // Calculate J(q)
            jacobian_ << -l1_ * sin(joint_positions_(0)) - l2_ * sin(joint_positions_(0) + joint_positions_(1)), -l2_ * sin(joint_positions_(0) + joint_positions_(1)),
                         l1_ * cos(joint_positions_(0)) + l2_ * cos(joint_positions_(0) + joint_positions_(1)),  l2_ * cos(joint_positions_(0) + joint_positions_(1));

            // Calculate J'(q,q')
            jacobian_derivative_ << -l1_ * cos(joint_positions_(0)) * joint_velocities_(0) - l2_ * cos(joint_positions_(0) + joint_positions_(1)) * (joint_velocities_(0) + joint_velocities_(1)),
                                     -l2_ * cos(joint_positions_(0) + joint_positions_(1)) * (joint_velocities_(0) + joint_velocities_(1)),
                                     -l1_ * sin(joint_positions_(0)) * joint_velocities_(0) - l2_ * sin(joint_positions_(0) + joint_positions_(1)) * (joint_velocities_(0) + joint_velocities_(1)),
                                     -l2_ * sin(joint_positions_(0) + joint_positions_(1)) * (joint_velocities_(0) + joint_velocities_(1));
        ...
        Eigen::MatrixXd differential_kinematics()
        {
            ...
            x_dot << jacobian_(0, 0) * joint_velocities_(0) + jacobian_(0, 1) * joint_velocities_(1),
                     jacobian_(1, 0) * joint_velocities_(0) + jacobian_(1, 1) * joint_velocities_(1);
