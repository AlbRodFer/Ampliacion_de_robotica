    ...
    double lambda)
{
    ...
    //Obtenemos los valores de posición iniciales y finales e interpolamos
    Eigen::Vector3d p_eig = start_pose.block<3,1>(0,3) + lambda*(end_pose.block<3,1>(0,3) - start_pose.block<3,1>(0,3));

    p_interp.setValue(p_eig.x(),p_eig.y(),p_eig.z());

    //Obtenemos los quaterniones con las matrices de rotación de la pose inicial (q_a) y final (q_b)
    tf2::Quaternion q_a = rot2Quat(start_pose.block<3,3>(0,0));
    tf2::Quaternion q_b = rot2Quat(end_pose.block<3,3>(0,0));

    //Creamos el quaternion de paso entre q_a y q_b
    tf2::Quaternion q_c = InverseQuaternion(q_a)*q_b;

    //Obtenemos el angulo y eje de giro asociado a q_c
    double theta = 2*acos(q_c.getW());
    tf2::Vector3 n(
        q_c.x() / sin(theta / 2.0),
        q_c.y() / sin(theta / 2.0),
        q_c.z() / sin(theta / 2.0)
    ...
    //Calculamos el q_rot con la ley temporal lambda
    ...
    q_rot.setValue(
        n.x()*sin(theta_interp/2.0),
        n.y()*sin(theta_interp/2.0),
        n.z()*sin(theta_interp/2.0),
        cos(theta_interp/2.0)
    );
    //Interpolamos
    q_interp = q_a*q_rot;
    ...
    double t)
{
    ...
    //Copio los valores de posicion en un vector de tf2 para cada una de las poses
    ...
    {
        //lambdas de la interpolacion de posicion y orientacion
        double lambda_1 = pow((tau - t),2)/(4*tau*T);
        double lambda_2 = pow((tau + t),2)/(4*tau*T);

        //quaternios de las poses
        tf2::Quaternion q_0 = rot2Quat(pose_0.block<3,3>(0,0));
        tf2::Quaternion q_1 = rot2Quat(pose_1.block<3,3>(0,0));
        tf2::Quaternion q_2 = rot2Quat(pose_2.block<3,3>(0,0));

        //quaternions de paso
        tf2::Quaternion q_01 = InverseQuaternion(q_0)*q_1;
        tf2::Quaternion q_12 = InverseQuaternion(q_1)*q_2;
        

        //obtenemos los angulos de giro y los ejes de giro asociados a los quaternions de paso
        double theta_01 = 2*acos(q_01.getW());
        double theta_12 = 2*acos(q_12.getW()); 

        //obtenemos los ejes de giro asociados a los quaternions de paso
        tf2::Vector3 n_01(
            q_01.x() / sin(theta_01 / 2.0),
            q_01.y() / sin(theta_01 / 2.0),
            q_01.z() / sin(theta_01 / 2.0)
        );

        tf2::Vector3 n_12(
            q_12.x() / sin(theta_12 / 2.0),
            q_12.y() / sin(theta_12 / 2.0),
            q_12.z() / sin(theta_12 / 2.0)
        );

        //calculamos los quaternions de paso para cada una de las interpolaciones
        double theta_k1 = -lambda_1*theta_01;
        double theta_k2 = lambda_2*theta_12;

        
        tf2::Quaternion q_k1 = tf2::Quaternion(
            n_01.x()*sin(theta_k1/2.0),
            n_01.y()*sin(theta_k1/2.0),
            n_01.z()*sin(theta_k1/2.0),
            cos(theta_k1/2.0)
        );

        tf2::Quaternion q_k2 = tf2::Quaternion(
            n_12.x()*sin(theta_k2/2.0),
            n_12.y()*sin(theta_k2/2.0),
            n_12.z()*sin(theta_k2/2.0),
            cos(theta_k2/2.0)
        );

        //calculamos delta_p1 y delta_p2 para la interpolacion de posicion
        tf2::Vector3 delta_p1 = p_1 - p_0;
        tf2::Vector3 delta_p2 = p_2 - p_1;
        
        //interpolamos posicion y orientacion
        p_interp = p_1 - lambda_1*delta_p1 + lambda_2*delta_p2;
        q_interp = q_1*q_k1*q_k2;
    }
