...
private:
    ...
    void controlLoop()
    {
        if (!measured_data_) {
            ...
            // 1. Calculamos la proporción (argumento del arco coseno)
            double acos_ratio = corridor_width_ / (lateral_distance + 1e-6);
            
            // 2. PROTECCIÓN CONTRA NaN: Limitamos el valor al máximo permitido matemáticamente (1.0)
            if (acos_ratio > 1.0) {
                acos_ratio = 1.0;
            } else if (acos_ratio < -1.0) {
                acos_ratio = -1.0; 
            }

            // 3. Calculamos la magnitud y el error final de forma segura
            double theta_mag = std::acos(acos_ratio); 
            double theta = theta_mag * theta_sign_; 
            
            RCLCPP_INFO(this->get_logger(), "Lateral Distance=%.2f m, Orientation Error=%.2f rad", lateral_distance, theta);

            /////////////////// CONTROL CODE ///////////
            
            // Primero calculamos las coordenadas locales del punto para poder calcular la distancia y el error
            // de orientación de manera más apropiada
            ...
            // Calculamos distancia y error de orientación
            double distance = sqrt((pow(xpl,2) + pow(ypl,2)));
            
            // PROTECCIÓN EXTRA: Evitamos división por cero si distance es muy cercana a 0
            double gamma = (2.0*ypl)/(pow(distance,2) + 1e-6);

            // Calculamos la velocidad de cada rueda siguiendo los parámetros declarados de nuestro Pioneer
            ...
            // En base a la velocidad de las ruedas calculamos la velocidad linear y angular global de nuestro robot
