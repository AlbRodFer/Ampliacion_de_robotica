...
private:
    ...
    void controlLoop()
    {
        ...
        double xpl = cos(current_theta_)*(goal_x_ - current_x_) + sin(current_theta_)*(goal_y_ - current_y_);
        double ypl = -sin(current_theta_)*(goal_x_ - current_x_) + cos(current_theta_)*(goal_y_ - current_y_);

        //Calculamos distancia y error de orientación
        double distance = sqrt(pow(xpl,2) + pow(ypl,2));
        double error_theta = atan2(ypl,xpl);

        //Realizamos el control proporcional
        double gamma = control_gain_*error_theta;
